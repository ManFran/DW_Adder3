...
/**	\file	ordeviceTimeCode_layout.h
*	Contains the declaration for the layout
*	device class, ORDeviceTimeCodeLayout.
*/
...
{
	//--- FiLMBOX declaration.
	FBDeviceLayoutDeclare( ORDeviceTimeCodeLayout, FBDeviceLayout );
...
private:
	void EventTakeRecordingTestClick( HISender pSender, HKEvent pEvent );
	void EventStoryRecordingTestClick( HISender pSender, HKEvent pEvent );
	void EventUIIdle( HISender pSender, HKEvent pEvent );
	void RecordingTest(bool pTakeRecording);

	ORDeviceTimeCode*		mDevice;					//!< Handle onto device.
	FBButton				mTakeRecordingTest,mStoryRecordingTest;
	FBPropertyAnimatable*	mMarkerTCProp;
};

#endif /* __ORDEVICE_TIMECODE_LAYOUT_H__ */
